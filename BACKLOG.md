...
**Síntoma:**
Al intentar iniciar sesión un cliente que aún no ha confirmado su email, la app mostraba "Ha ocurrido un error inesperado. Inténtalo de nuevo".

**Causa raíz:**
`mapSupabaseAuthError` no tenía caso para el código `email_not_confirmed` de Supabase Auth y caía al fallback genérico.

**Fix aplicado:**
- Añadido caso `EMAIL_NOT_CONFIRMED` en `AuthErrorCode` y en `mapSupabaseAuthError`.
- Nuevo string `errorEmailNotConfirmed` explicando al cliente que debe confirmar el email.
- 3 tests nuevos en `AuthError.test.ts`.
...
**Síntoma:**
Al subir un `.jpg` desde la pantalla de documentos aparecía `Network request failed`.

**Causa raíz:**
`DocumentRemoteRepository.uploadFile` usaba `fetch(localUri).blob()`. En React Native, el Blob resultante es incompatible con el SDK de Supabase Storage.

**Fix aplicado:**
- `DocumentRemoteRepository.uploadFile` ahora usa `FileSystem.readAsStringAsync(uri, { encoding: 'base64' })` + conversión a `Uint8Array`, mismo patrón validado en `RecipeRemoteRepository.uploadImage`.
- Extensión extraída antes del `?` para evitar corrupción con URIs con query string.
- 6 tests en `DocumentRemoteRepository.test.ts`.
...
**Síntoma:**
Al entrar en la pantalla de planes nutricionales: `Could not find a relationship between meals and meal_recipes in the schema cache`.

**Causa raíz:**
La tabla `meal_recipes` no existía en la base de datos. La query en `NutritionRemoteRepository` hacía join a una tabla inexistente.

**Fix aplicado:**
- Migración `create_meal_recipes_table`: tabla `meal_recipes` con FK `meal_id → meals.id` (CASCADE) y FK `recipe_id → recipes.id`, RLS habilitado, política para que el coach gestione los registros a través de sus planes nutricionales.
