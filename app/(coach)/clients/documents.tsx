...
export default function ClientDocumentsScreen() {
  ...
  useFocusEffect(
    useCallback(() => {
      if (user?.id && athleteId) fetchDocuments(user.id, athleteId);
    }, [user?.id, athleteId]),
  );
      ...
      <View style={styles.container}>
        ...
        {isLoading ? (
          ...
          <View style={styles.center}>
            ...
            <Text style={styles.emptySubtitle}>{Strings.docEmptySubtitle}</Text>
          </View>
        ) : (
          <FlatList
            data={documents}
...
const styles = StyleSheet.create({
  ...
  cardIconText: { fontSize: 9, fontWeight: '800', color: Colors.primary, letterSpacing: 0.5 },
  cardContent:  { flex: 1 },
  ...
  openButtonText: { color: Colors.primary, fontSize: FontSize.xs, fontWeight: '700' },
