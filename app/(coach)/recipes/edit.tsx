...
export default function EditRecipeScreen() {
  ...
  // Populate form once currentRecipe loads
  useFocusEffect(
    useCallback(() => {
      if (currentRecipe && currentRecipe.id === id && !initialized) {
        setName(currentRecipe.name);
        setInstructions(currentRecipe.instructions ?? '');
        setTags(currentRecipe.tags);
        setKeepImage(!!currentRecipe.imagePath);
        setIngredients(
  ...
  const handleAddIngredient = () => {
    ...
    if (ingredients.some((i) => i.foodId === pendingFood.id)) {
      setIngredients((prev) =>
        prev.map((i) => i.foodId === pendingFood.id ? { ...i, quantityG: qty } : i),
      );
    } else {
  ...
  const handleSubmit = async () => {
    ...
    const oldImagePath = currentRecipe?.imagePath ?? null;
    // If removing image: pass null explicitly; if keeping original: pass undefined (no change)
    const imagePath = imageUri
      ? undefined                    // new image — store will upload and set the path
      : keepImage
        ? currentRecipe?.imagePath   // no change
    ...
    const result = await updateRecipe(
      ...
      imageUri ?? undefined,
      imageUri ? oldImagePath : undefined,
    );
    ...
    <SafeAreaView style={styles.safe}>
      <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled">
        ...
        <View style={styles.header}>
          ...
          <Text style={styles.headerTitle}>{Strings.recipeFormEditTitle}</Text>
        ...
        <View style={styles.addIngredientBlock}>
          <TouchableOpacity style={styles.foodSelector} onPress={() => setFoodPickerVisible(true)}>
